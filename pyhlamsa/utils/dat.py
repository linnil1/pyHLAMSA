...
def apply_dat_info_on_msa(msa: Genemsa, dat: DatType, seq_type: str = "gen") -> Genemsa:
    ...
    for allele_name, seq in msa.alleles.items():
        ...
        if not all(i in "ATCG-" for i in seq):
            logger.warning(
                f"Ignore {allele_name}, msf length contains invalid character"
        ...
        for block_info in dat[allele_name]:
            ...
            block_cord[block_name][0] = min(
                block_cord[block_name][0], seq_cord[start - 1]
            )
            block_cord[block_name][1] = max(
                block_cord[block_name][1], seq_cord[end - 1] + 1
            )
