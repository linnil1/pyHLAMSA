...
def add_parser() -> argparse.ArgumentParser:
    ...
    parser_download.add_argument("--family", required=True,
                                 choices=["hla", "kir", "cyp"],
    ...
    parser_download.add_argument("name",
                                 help="Prefix of output name")
    parser_download.add_argument("--include-genes", nargs="+", default=None,
    ...
    parser_view = subparser.add_parser(
            ...
            formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser_view.add_argument("input_name",
    ...
    parser_view.add_argument("--region", nargs="+",
                             help="Specific the region (split by space)"
                                  "Example: intron1 exon2")
    parser_view.add_argument("--position", type=str,
                             help="Specific the position. Example: 100-400")
    parser_view.add_argument("--include-alleles", nargs="+", default=None,
                             help="Show these alleles only")
    parser_view.add_argument("--exclude-alleles", nargs="+",
                             help="Remove some alleles")
    parser_view.add_argument("--fasta-msa", action="store_true",
                             help="Save to fasta (All sequences are equal length)")
    parser_view.add_argument("--fasta-gapless", action="store_true",
                             help="Save to fasta")
    parser_view.add_argument("--bam", action="store_true",
                             help="Save to bamfile")
    parser_view.add_argument("--gff", action="store_true",
                             help="Save to gff")
    parser_view.add_argument("--vcf", action="store_true",
                             help="Save to vcf")
    return parser
...
def extract_msa(args) -> Genemsa:
    ...
    if args.exclude_alleles:
        msa = msa.select_allele(list(set(msa.list_alleles())
                                     - set(args.exclude_alleles)))
    if args.region:
...
def write_to_files(args, msa: Genemsa):
    ...
    if not args.name:
        if (args.bam or args.gff or args.fasta_gapless
                or args.fasta_msa or args.vcf or args.save):
            raise ValueError("No output name specific")
        return
    if args.input_name == args.name:
        raise ValueError("output name should not same as input name,"
